...
async function carregarMinhasDuvidas() {
    ...
    listaMinhasDuvidas.innerHTML = '<li>Carregando minhas dúvidas...</li>';

    try {

        const responseDuvidas = await fetch(`${API_DUVIDAS}?autorId=${currentUserId}`);
        const duvidas = await responseDuvidas.json();

        listaMinhasDuvidas.innerHTML = ''; 

        if (duvidas.length === 0) {
            listaMinhasDuvidas.innerHTML = '<li>Você ainda não enviou nenhuma dúvida.</li>';
            return;
        }
        ...
        for (const duvida of duvidas) {
            ...
            if (comentarios.length === 0) {
                ...
                comentarios.forEach(comentario => {
                    // Marca se o comentário é do monitor
            ...
            itemDuvida.innerHTML = `
                ...
                <small>Matéria: ${duvida.materia.nome}</small>
...
// Inicialização da Página do Aluno
